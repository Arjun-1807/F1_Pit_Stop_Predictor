...
class PredictRequest(BaseModel):
    circuit_key: str = Field(..., description="Circuit identifier (e.g. BAH)")
    LapTime: float = Field(..., description="Current lap time in seconds")
    LapNumber: int = Field(..., description="Current lap number in the race")
    Stint: int = Field(..., description="Current stint number for the driver")
    Sector1Time: float = Field(..., description="Sector 1 time in seconds")
    Sector2Time: float = Field(..., description="Sector 2 time in seconds")
    Sector3Time: float = Field(..., description="Sector 3 time in seconds")
    TyreLife: float = Field(..., description="Laps completed on current tyre set")
    Position: int = Field(..., description="Current race position of the driver")
    Season: int = Field(..., description="Season/Year (e.g. 2024)")
    sc_active: int = Field(..., description="Safety Car active (1) or not (0)")
    vsc_active: int = Field(..., description="Virtual Safety Car active (1) or not (0)")
    red_flag: int = Field(..., description="Red flag active (1) or not (0)")
    laps_remaining: int = Field(..., description="Remaining laps in the race")
    race_pct: float = Field(..., description="Percentage of race distance completed")
    stint_lap: int = Field(..., description="Laps completed in current stint")
    lap_time_roll3: float = Field(..., description="Rolling average of last 3 laps")
    lap_time_delta_best: float = Field(..., description="Difference from stint personal best")
    lap_time_delta_prev: float = Field(..., description="Difference from previous lap time")
    compound_enc: int = Field(..., description="Encoded tyre compound")
    fresh_tyre: int = Field(..., description="Fresh tyre (1) or used tyre (0)")
